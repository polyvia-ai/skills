...
Requires a `poly_...` API key from **app.polyvia.ai → API** (in the sidebar). Each key is permanently bound to the workspace (personal or one organization) you were in when you created it — the MCP server reads/writes only that workspace. Switch workspace and create a separate key if you need to operate on more than one.
